#!/bin/bash
# Resume the four PAUSED ablation rows on GPU1 only (GPU1 freed when the
...
# All four rows, sequential, GPU1 only.
run_track 1 baseline    train_loso_genbio.py
run_track 1 meancenter  train_loso_meancenter_genbio.py
run_track 1 sa_stratval train_loso_sa_genbio.py --val_split stratified
run_track 1 e09         train_loso_e09_genbio.py

echo "REMAINING(GPU1) ROWS COMPLETE $(date +%T)"
