...
reuses shared utilities from ``CHIME_MIL.utils.common``, and keeps the
...
from dataset_genbio_shim import UniCLAMDatasetFixed  # noqa: E402
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "results" / "results_loso_soft_assign_genbio_20260421"
LAMBDA_PATCH = 0.2
LAMBDA_REGION = 0.2
LAMBDA_GRAPH = 0.3
...
def main() -> None:
    ...
    for fold_idx, hospital in enumerate(runtime.hospitals, 1):
        ...
        # so a second worker (sa_stratval Site_E launched on GPU3) and the
        # GPU1 driver never retrain or overwrite the same fold. Absent => run.
        if not args.eval_only:
            _done = os.path.join(runtime.output_dir, f"fold_{fold_idx}_{hospital}", "result.json")
